...
def shorten_line_length(string):
    max_len = 1000
    return "\n".join([string[i*max_len:(i+1)*max_len] for i in range(0, math.ceil(len(string)/max_len))])
...
def add_images(tex, images):
    ...
    for image in images:
        ...
        graphics += "\\newpage\n\\fbox{{\\includegraphics[width=\\textwidth,height=\\textheight,keepaspectratio]{{\\jobname-tmp{0}.pdf}}}}".format(i)
        i += 1
...
def add_signature(tex, signature):
    ...
    immediate = "\\immediate\\write18{{base64 -d \\jobname.embedded{0} > \\jobname-tmp{0}.pdf}}\n".format("signature")
    graphics = "\\newpage\n\\fbox{{\\includegraphics[width=6cm]{{\\jobname-tmp{0}.pdf}}}}".format("signature")
